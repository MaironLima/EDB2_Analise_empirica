...
# --- NOVO: linha de referência O(n) ---------------------------------------
# Reta teórica y = C * n (crescimento LINEAR), usada como referência para
# visualizar se o crescimento medido do AES/RSA se aproxima ou se afasta de
# um comportamento O(n). A constante C é ajustada (ancorada) para que a
# reta passe pelo primeiro ponto do AES, só para ficar na mesma "régua"
# visual do gráfico - o que importa é a INCLINAÇÃO da reta em escala
# log-log, não sua posição exata.
primeiro_n = media_aes.index[0]
C = media_aes.values[0] / primeiro_n
n_ref = media_aes.index  # mesmos tamanhos de mensagem usados no eixo x
tempo_On = C * n_ref

plt.plot(n_ref, tempo_On, linestyle='--', linewidth=2.5,
         color='#16a34a', label='O(n) - referência linear')
# ----------------------------------------------------------------------------
...
plt.tight_layout()
plt.savefig('grafico_assintotico_2.png', dpi=300, bbox_inches='tight')
print("Gráfico salvo com sucesso: grafico_assintotico_2.png")